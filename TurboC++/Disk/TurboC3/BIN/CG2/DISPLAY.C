...
int op,df_op[50],we,ws,he,hs,w,h,top,erase_flag;
float df_penx,df_peny,df_x[50],df_y[50],frame_penx,frame_peny,x1,y1;
...
{
	int p,twody,twodx,twodxdy,twodydx,dx,dy,xend,yend,x,y;
	float m;
	dx = abs(xa-xb);
	dy = abs(ya-yb);
if(dx>dy)
